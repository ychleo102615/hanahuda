...
前端需要實作基本的業務邏輯判斷，提供即時反饋並減少無效操作。這些邏輯是輔助性的，伺服器擁有最終權威：

**配對與驗證邏輯（P1 - 必須實作）**
- 識別可配對的牌：判斷手牌與場牌的配對關係（相同月份）
- 高亮顯示目標：玩家選擇手牌時，自動高亮場上所有可配對的牌
- 客戶端預驗證：在發送命令前檢查操作合法性，提前給予錯誤提示
- 雙重配對判斷：預測是否會觸發「需要選擇目標」的流程
- 手牌可打性標示：標示哪些手牌當前可打出（有配對機會 vs. 無配對直接進場）

**卡片資訊查詢（P1 - 必須實作）**
- 卡片ID解析：解析 MMTI 格式，提取月份、類型、索引
- 卡片屬性查詢：返回月份、類型、點數、顯示名稱
- 卡片分組與排序：按月份、類型、點數分類和排序（用於 UI 顯示）
- 卡片ID驗證：檢查 MMTI 格式合法性（月份 01-12、類型 1-4、索引 1-4）

**役種相關邏輯（P2 - 重要）**
- 當前役種檢測：實時計算玩家已獲得牌形成的役種
- 役種進度提示：顯示「距離赤短還差1張」、「已收集豬鹿蝶中的豬和鹿」
- 預測分數倍率：根據當前役種、Koi-Koi 次數計算潛在得分
- Koi-Koi 決策建議：基於當前分數、牌堆剩餘、對手狀況給予建議
...
- 局進度視覺化：顯示當前局進行到幾%
- 分數差距分析：計算與對手的分數差，判斷策略傾向
- 出牌歷史記錄：記錄本局的所有出牌順序（對應「遊戲記錄」功能）

**規則輔助與教學（P3 - 輔助功能）**
- 規則提示：根據當前情況顯示相關規則說明
- 役種說明：Hover 某役種時顯示詳細說明和所需卡片
- 新手引導：標示「建議打這張牌」、「這張牌可能形成役種」（可選）

**狀態驗證與除錯（開發階段）**
- 狀態一致性檢查：比對本地狀態與伺服器 SSE 事件，偵測不一致
- 重複卡片偵測：檢查場上、手牌、已獲得牌中是否有重複ID

**關鍵原則**：
- ✅ 所有前端邏輯必須是純函數（無副作用），不依賴任何 UI 框架
- ✅ 前端邏輯僅用於提升使用者體驗，即使計算錯誤也不會破壞遊戲規則
- ✅ 伺服器擁有最終權威，所有操作由後端驗證
- ✅ 前端 Domain Layer 遵循 Clean Architecture 原則（詳見第 5 節）
