...
前端遵循 **Clean Architecture 三層架構**（Domain → Application → Adapter），實作以下核心功能：

---

##### A. Domain Layer - 遊戲業務邏輯

**核心職責**：封裝花札規則與遊戲邏輯，提供純函數運算，不依賴任何框架或外部狀態。

**1. 卡片核心邏輯（P1 - 必須實作）**
- **卡片ID解析**：解析 MMTI 格式，提取月份（01-12）、類型（1-4）、索引（1-4）
- **卡片屬性查詢**：根據ID返回月份、類型名稱、點數、顯示名稱
- **卡片驗證**：檢查ID格式合法性，拒絕無效輸入
- **卡片分組與排序**：按月份、類型、點數分類（用於UI顯示）

**2. 配對驗證邏輯（P1 - 必須實作）**
- **識別可配對的牌**：判斷手牌與場牌的配對關係（相同月份）
- **多目標判斷**：當手牌可與場上多張牌配對時，返回所有可選目標列表
- **客戶端預驗證**：在發送命令前檢查操作合法性（例如：月份不符、卡片不存在）
- **配對狀態分類**：區分「無配對」、「單一配對」、「雙重配對」三種情況

**3. 役種檢測邏輯（P2 - 重要）**
- **實時役種檢測**：根據玩家已獲得牌，計算當前形成的役種（光牌系、短冊系、種牌系、かす系）
- **役種進度計算**：顯示「距離赤短還差1張」、「已收集豬鹿蝶中的豬和鹿」等資訊
- **分數預測**：根據當前役種、Koi-Koi 次數計算潛在得分與倍率
- **役種衝突判斷**：識別互斥役種（例如四光與雨四光）

**4. 對手分析邏輯（P2 - 重要）**
- **對手役種預測**：根據對手已獲得牌，計算可能形成的役種
- **威脅度評估**：標示對手「距離五光還差2張」等警示資訊
- **卡片分布統計**：分析對手收集的牌型（幾張光、幾張種、幾張短、幾張かす）

**5. 遊戲進度計算（P3 - 輔助功能）**
- **剩餘回合計算**：根據牌堆剩餘張數和手牌數，計算還能進行幾回合
- **局進度百分比**：計算當前局進行到幾%（用於進度條顯示）
- **分數差距分析**：計算與對手的分數差，判斷策略傾向（激進/保守）
...
##### B. Application Layer - 狀態與流程控制

**核心職責**：管理遊戲狀態、協調 Domain Layer 與 Adapter Layer、控制遊戲流程。

**1. 遊戲狀態管理**
- 維護遊戲狀態（FlowStage、卡牌位置、分數、當前玩家、回合數）
- 根據 FlowStage 控制 UI 可操作性（`AWAITING_HAND_PLAY` / `AWAITING_SELECTION` / `AWAITING_DECISION`）
- 調用 Domain Layer 進行配對檢查、役種檢測等運算
- 同步處理 SSE 事件以更新本地狀態

**2. 玩家操作流程**
- **打出手牌**：
  1. 調用 Domain Layer 驗證是否可打出
  2. 發送 `TurnPlayHandCard` 命令到後端
  3. 等待 SSE 事件確認並更新 UI
- **選擇配對目標**：
  1. 調用 Domain Layer 取得可選目標列表
  2. 顯示選擇 UI 讓玩家選擇
  3. 發送 `TurnSelectTarget` 命令
- **Koi-Koi 決策**：
  1. 調用 Domain Layer 計算當前役種與得分
  2. 顯示決策選項（繼續/結束）
  3. 發送 `RoundMakeDecision` 命令

**3. SSE 事件處理**
- 監聽所有遊戲事件（`GameStarted`、`RoundDealt`、`TurnCompleted`、`SelectionRequired`、`DecisionRequired` 等）
- 根據事件更新狀態並觸發 UI 動畫
- 處理斷線重連的 `GameSnapshotRestore` 事件（快照模式恢復完整狀態）

---

##### C. Adapter Layer - 通訊與錯誤處理

**核心職責**：與後端 API 通訊、處理網路錯誤、管理 SSE 連線。

**1. 遊戲初始化**
- 透過 REST API 發送 `GameRequestJoin` 命令（加入新遊戲或重連）
- 建立 SSE 連接到 `/api/v1/games/{gameId}/events`
- 保存 `session_token` 到 `localStorage` 用於斷線重連

**2. 錯誤處理**
- **操作錯誤**：處理 `TurnError` 事件（無效操作、流程錯誤等），顯示友善的錯誤提示
- **網路斷線**：偵測 SSE 連線中斷，自動重連（指數退避策略：1s → 2s → 4s → 8s）
- **快照恢復**：重連成功後接收 `GameSnapshotRestore` 事件，恢復完整遊戲狀態
...
- `GET /api/v1/games/{gameId}/events`（SSE）：接收伺服器事件流

---

**詳細規格請參考**：
- 完整命令與事件規格：[game-flow.md](./game-flow.md)
- 前端狀態流程：[frontend-state-flow.md](./frontend-state-flow.md)
- 後端功能與 API：[2.2 節](#22-後端功能)
- 通訊架構設計：[4.2 節](#42-通訊流程圖)
