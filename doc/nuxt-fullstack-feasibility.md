...
經過評估後，專案已採用 Nuxt 4 Nitro 作為後端實作方案。
...
1. **全棧 TypeScript**：前後端使用相同語言，提升開發效率
2. **共用類型定義**：`shared/contracts/` 目錄共用 TypeScript 類型
3. **單體部署簡化**：前後端整合於同一專案，降低部署複雜度
4. **開發效率**：快速迭代 MVP，無需維護兩套系統

### 實作差異

與原始可行性分析的差異：
- 使用 Drizzle ORM 取代 Prisma（更輕量、SQL-like 語法）
- 未實作 Local Game BC（離線模式），專注於線上模式
