...
- [ ] 1.2 建立 `session-db-store.ts`：從 `drizzle-session-store.ts` 提取邏輯，移除 `SessionStorePort` 繼承，class 改名為 `SessionDbStore`
- [ ] 1.3 建立 `session-memory-store.ts`：從 `in-memory-session-store.ts` 提取邏輯，移除 `SessionStorePort` 繼承，class 改名為 `SessionMemoryStore`，保留 `size` getter 與 `clear()` 供 cleanup 使用

## 2. 建立 CachingSessionStore

- [ ] 2.1 建立 `adapters/session/caching-session-store.ts`，實作 `SessionStorePort`
- [ ] 2.2 實作 `findById`：memory hit 直接回傳；miss 則 fallback DB，結果存入 memory
...
- [ ] 2.7 實作 `cleanupExpired`：分別清理 memory 與 DB，回傳總刪除數量
...
- [ ] 4.1 更新 `plugins/sessionCleanup.ts`：移除 `getSessionStore()` 的孤立全域單例呼叫，改呼叫 `getIdentityContainer().sessionStore.cleanupExpired()`
