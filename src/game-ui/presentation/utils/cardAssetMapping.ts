 ...
 * 支援多種渲染器使用（DOM/PIXI）
 *
 * 注意: 此檔案屬於 game-ui BC,使用 CardDefinition 而非 Card 實體
 */
...
/**
 * 從 CardDefinition 獲取對應的 SVG 檔案名稱
 * @param card 花牌定義
 * @returns SVG 檔案名稱（不含副檔名）
 */
export function getCardSvgName(card: CardDefinition): string {
 ...
 */
export function getCardSvgPath(card: CardDefinition, basePath: string = '/src/assets/icon/'): string {
 ...
 * @returns symbol ID
 */
export function getCardSvgSymbolId(card: CardDefinition, prefix: string = 'icon'): string {
  const svgName = getCardSvgName(card)
