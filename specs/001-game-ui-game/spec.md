...
- 當牌堆翻出的牌與場上有多張同月份配對時,系統應如何選擇? → game-ui 會要求玩家在限定時間內選擇一張場牌；若時間到期仍未選擇,game-engine 會按點數優先順序(光>種>短>カス)自動選擇,同類型則按場牌出現順序選擇第一張
...
- 當整合事件因為某種原因遺失或順序錯誤(如網路問題),系統應如何處理? → game-ui 維護事件序號,檢測到序號不連續時立即暫停處理新事件並向 game-engine 請求完整狀態快照,同步完成後恢復事件處理
- 當 game-ui 的本地狀態與 game-engine 不同步時,系統應如何處理? → 提供「重新同步」功能按鈕,讓玩家可手動觸發向 engine 請求當前完整遊戲狀態快照
- 當所有手牌和牌堆都用盡且雙方都沒有役種時,系統應如何處理? → 判定為平局,顯示「本回合平局」訊息,本回合無人得分,直接進入下一回合或結束遊戲(如已達最大回合數)
- 當玩家在限時選擇配對期間斷線重連時,系統應如何處理? → 如果選擇時限尚未到期,恢復選擇介面並重新計時;如已超時,按自動選擇規則處理並繼續遊戲
...
- **FR-004**: 系統必須定義一套完整的整合事件在 shared 資料夾中,包含 11 種核心事件:GameInitialized、CardPlayed、MatchSelected、KoikoiDeclared、RoundEnded、GameEnded、GameAbandoned、DeckCardRevealed、MatchSelectionRequired、MatchSelectionTimeout、PlayerTurnChanged 等事件
- **FR-005**: 整合事件的資料結構必須設計為可移植到非 TypeScript 環境(如 Protocol Buffers),欄位定義應避免依賴 TypeScript 特定型別
- **FR-006**: 除了 GameInitialized 和重新同步情境,所有整合事件必須只包含增量變化資訊,不傳送完整的遊戲狀態快照;每個增量事件的序列化大小必須 <1KB
...
- **FR-008**: 系統必須正確實施花牌來來的所有規則,包含:
  ...
  - 10 種標準役種的判定邏輯和計分:
    ...
    * 雨四光 (7分)：4張光牌且包含11月雨光
    * 三光 (5分)：3張光牌且不含11月雨光
    * 猪鹿蝶 (5分)：7月豬、10月鹿、6月蝶種牌
    * 赤短 (5分)：松竹梅的短冊(1、2、3月)
    * 青短 (5分)：6、9、10月的青短冊
    * 種 (1分)：9張以上種牌，每多1張加1分
    * 短 (1分)：9張以上短冊，每多1張加1分
    * カス (1分)：9張以上カス牌，每多1張加1分
  - 11 月雨光的特殊規則(3 張光含雨光不成立三光、4 張光含雨光為雨四光而非四光)
  - Koi-Koi 宣告的加倍計分規則
  - 平局判定(所有手牌用盡且雙方都沒有役種時，本回合無人得分，不影響總分)
...
- **FR-013**: 系統必須提供事件序號機制,讓 game-ui 能檢測事件遺失;一旦檢測到序號不連續,game-ui 必須立即暫停處理後續事件並向 game-engine 請求完整狀態快照,同步完成後才恢復事件處理
- ~~**FR-014**: 整合事件必須包含基礎的上下文資訊,使 game-ui 可以進行動畫呈現,包含:出牌的來源(玩家手牌/牌堆)、配對數量(0, 2, 4)~~  暫時不處理
- **FR-015**: 系統必須在 shared 資料夾中定義整合事件的 TypeScript 型別定義,作為兩個 BC 的契約
- **FR-016**: 當牌堆翻牌與場上有多張配對時,game-ui 必須提供限時選擇介面讓玩家選擇要配對的場牌
- **FR-017**: 當玩家在限定時間內未選擇配對的場牌,game-engine 必須按點數優先順序(光>種>短>カス)自動選擇場牌,同類型卡牌則按場牌出現順序選擇第一張,並繼續遊戲流程
- **FR-018**: game-ui 和 game-engine 必須各自實作花牌配對尋找的領域服務,以支援各自的職責(UI 顯示選項、Engine 自動選擇)
- **FR-019**: 系統必須在整合事件發佈和接收時記錄日誌到瀏覽器 console,包含事件類型、序號、時間戳、關鍵欄位,開發環境顯示詳細日誌,production 環境可透過環境變數(如 VITE_LOG_EVENTS)關閉以提升效能
...
- **Shared**:
  - **EventBus**: 輕量級事件匯流排,提供 subscribe/unsubscribe/publish 方法,支援型別安全的事件訂閱,在單機模式下同步傳遞事件物件,內建日誌記錄功能
  - **EventLogger**: 事件日誌記錄器,負責將事件資訊格式化並輸出到 console,根據環境變數控制日誌詳細程度(development: 完整事件內容,production: 僅關鍵欄位或關閉)
  ...
  - **CardMatchingService**: 花牌配對尋找領域服務介面,定義配對規則(同月份)及自動選擇優先順序邏輯(點數優先:光>種>短>カス,同類型按場牌出現順序)
...
- **SC-008**: 事件遺失檢測機制可在接收到下一個事件時立即發現序號不連續(<1ms),自動暫停事件處理並在 50ms 內完成完整狀態同步(單機模式)
...
- **SC-011**: 當牌堆翻牌遇到多重配對時,玩家可在 10 秒內完成選擇;若超時,遊戲自動繼續而不中斷;限時選擇UI響應時間 <100ms,超時檢測精確度 ±50ms

## Clarifications

### Session 2025-10-14

- Q: 整合事件在單機模式下的儲存與傳輸格式應如何設計? → A: 事件在記憶體中保持 TypeScript 物件,只在網路傳輸時序列化為 JSON
- Q: 單機模式下 game-engine 與 game-ui 之間的事件匯流排應使用何種實作方案? → A: 自定義輕量級 EventBus 類別(支援訂閱/發佈,記憶體內同步傳遞)
- Q: 當 game-ui 檢測到事件序號不連續(遺失事件)時,應採取何種同步策略? → A: 立即請求完整狀態快照並暫停接收新事件,直到同步完成
- Q: 當牌堆翻牌與場上有多張同月份牌可配對且玩家超時未選擇時,game-engine 自動選擇的優先順序規則應如何定義? → A: 按點數高低選擇(優先選擇光>種>短>カス),同類型則按場牌出現順序選擇
- Q: 整合事件的日誌與監控策略應如何設計,以支援開發除錯與日後的生產環境診斷? → A: 記錄到瀏覽器 console,production 環境可透過環境變數關閉詳細日誌

## Assumptions

- 假設目前專案使用的 Clean Architecture 分層結構將維持不變,只是將層級歸屬於不同的 BC
- 假設單機模式下的 AI 對手邏輯(OpponentAI)將歸屬於 game-engine BC
- 假設事件通訊在單機模式下使用自定義輕量級 EventBus 類別(實作訂閱/發佈模式,記憶體內同步傳遞),確保 game-engine BC 不依賴任何 UI 框架或狀態管理工具,事件以 TypeScript 物件形式直接傳遞
- 假設日後前後端分離時,game-engine 會部署為 WebSocket 或 HTTP Server,事件在網路傳輸時會序列化為 JSON 格式
...
- 假設整合事件日誌使用標準的 console.log/console.warn/console.error API,不依賴第三方日誌庫,日後前後端分離時 server 端可替換為專業日誌服務(如 Winston/Pino)
