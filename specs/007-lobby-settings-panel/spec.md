...
- **FR-013**: 確認退出後，系統必須結束遊戲會話並導航回首頁
- **FR-014**: 操作面板必須在使用者點擊面板外區域時自動關閉（滑回）
- **FR-015**: 操作面板必須包含關閉按鈕（X 圖示），點擊後關閉面板
- **FR-016**: 當使用者在遊戲中斷線並重連成功時，系統必須直接恢復到遊戲畫面，而不經過遊戲大廳
- **FR-017**: 操作面板的設計必須支援未來擴充（如新增「Settings」、「Rules」等選項），使用可擴展的清單結構
