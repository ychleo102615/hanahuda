...
- [X] T039 [P] [US3] 整合測試完整退出遊戲流程（API 呼叫 + 狀態清除 + 導航）於 front-end/tests/composables/useLeaveGame.spec.ts

### Implementation for User Story 3

- [X] T040 [US3] 實作遊戲 context 的「Leave Game」選項（透過 useLeaveGame composable）於 front-end/src/user-interface/adapter/composables/useLeaveGame.ts
- [X] T041 [US3] 實作「Leave Game」確認對話框於 ConfirmDialog.vue（新增獨立組件）
- [X] T042 [US3] 實作確認退出邏輯（調用 leaveGame API、清除狀態、中斷 SSE）於 useLeaveGame composable
- [X] T043 [US3] 整合 ActionPanel 至 GamePage.vue
...
- [X] T049 [P] 更新 protocol.md 新增 GameError 事件規格於 doc/shared/protocol.md
- [X] T050 [P] 程式碼清理與重構（移除 TODO 註解、優化命名）
- [X] T051 [P] 新增缺失的單元測試（目標覆蓋率 > 70%）
- [X] T052 [P] 樣式優化與響應式設計調整
- [X] T053 [P] 可訪問性改進（鍵盤導航、ARIA 標籤）
- [X] T054 執行 quickstart.md 驗證（手動測試完整流程）
- [X] T055 效能優化（動畫流暢度、記憶體洩漏檢查）
- [X] T056 [P] 錯誤訊息國際化準備（i18n keys）
