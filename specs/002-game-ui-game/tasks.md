...
- [ ] T001 確認當前在 `002-game-ui-game` 分支並執行基準測試
- [ ] T002 驗證當前測試通過率 (預期 >= 94%, 82/87 測試)
- [ ] T003 [P] 執行 TypeScript 編譯檢查確認已知的 7 個跨 BC 依賴違規
...
- [ ] T020 [P] [US2] 遷移 CardComponent.vue 從 src/ui/components/ 到 src/game-ui/presentation/components/CardComponent.vue 並更新 props 使用 CardDefinition
- [ ] T021 [P] [US2] 遷移 GameBoard.vue 從 src/ui/components/ 到 src/game-ui/presentation/components/GameBoard.vue 並更新使用 GameViewModel
- [ ] T022 [P] [US2] 遷移 PlayerHand.vue 從 src/ui/components/ 到 src/game-ui/presentation/components/PlayerHand.vue 並更新使用 PlayerViewModel
- [ ] T023 [P] [US2] 遷移其他 UI components (如 YakuDisplay, ScoreBoard 等) 到 src/game-ui/presentation/components/
- [ ] T024 [P] [US2] 遷移 cardAssetMapping.ts 到 src/game-ui/presentation/utils/cardAssetMapping.ts
- [ ] T025 [US2] 擴展 DIContainer 新增 game-ui BC 服務鍵和註冊方法在 src/infrastructure/di/DIContainer.ts
- [ ] T026 [US2] 實作 DIContainer.setupGameUIServices() 方法註冊所有 game-ui BC 元件
- [ ] T027 [US2] 實作 DIContainer.createWithGameUI() 靜態工廠方法
