 ...
 * @description
 * 取得日排行榜的 Use Case。
 * 從 Repository 取得當日排行榜資料並計算排名。
 *
 * @module server/leaderboard/application/use-cases/get-daily-leaderboard-use-case
 */
 ...
 * 取得日排行榜 Use Case
 */
export class GetDailyLeaderboardUseCase implements GetLeaderboardInputPort {
  constructor(
  ...
  async execute(request: GetLeaderboardRequest): Promise<GetLeaderboardResponse> {
    // 從 Repository 取得排行榜原始資料
    const rawData = await this.dailyScoreRepository.getLeaderboard('daily', request.limit)
    ...
    if (request.currentPlayerId) {
      ...
      if (!isInEntries) {
        // 玩家不在前 N 名內，額外取得其排名
        const rank = await this.dailyScoreRepository.getPlayerRank(request.currentPlayerId, 'daily')
        if (rank !== null) {
